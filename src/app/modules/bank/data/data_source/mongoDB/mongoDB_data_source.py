...
from app.modules.bank.data.models.account_model import AccountModel
from app.modules.bank.data.models.bank_model import BankModel
    ...
    def subscribe(self, params: SubscribeParams) -> bool:
        try:
            ...
            get_customer_account = next(
                (customer_account for customer_account in get_bank.accounts if customer_account.customer.phoneNumber == params.customer.phoneNumber), None)
            

            if get_customer_account:
                print("create account",get_customer_account)
                raise ServerException(
                    message=f"This number {get_customer_account.customer.phoneNumber} is already use")
